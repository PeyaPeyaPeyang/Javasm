...
public class InstructionEvaluatorDLoadN extends AbstractInstructionEvaluator<JALParser.JvmInsDloadNContext>
{
                                                     ...
                                                     JALParser.@NotNull JvmInsDloadNContext ctxt)
    {
        if (has(ctxt.INSN_DLOAD_0()))
            return InstructionEvaluateHelperXLoad.evaluateN(ctxt, compiler, EOpcodes.DLOAD, 0);
        else if (has(ctxt.INSN_DLOAD_1()))
            return InstructionEvaluateHelperXLoad.evaluateN(ctxt, compiler, EOpcodes.DLOAD, 1);
        else if (has(ctxt.INSN_DLOAD_2()))
            return InstructionEvaluateHelperXLoad.evaluateN(ctxt, compiler, EOpcodes.DLOAD, 2);
        else if (has(ctxt.INSN_DLOAD_3()))
            return InstructionEvaluateHelperXLoad.evaluateN(ctxt, compiler, EOpcodes.DLOAD, 3);
